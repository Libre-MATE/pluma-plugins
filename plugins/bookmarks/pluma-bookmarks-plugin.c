...
static void on_style_scheme_notify(GObject *object, GParamSpec *pspec,
                                   PlumaView *view);

static void on_delete_range(GtkTextBuffer *buffer, GtkTextIter *start,
                            GtkTextIter *end, gpointer user_data);

static void on_insert_text_before(GtkTextBuffer *buffer, GtkTextIter *location,
                                  gchar *text, gint len, InsertData *data);

static void on_begin_user_action(GtkTextBuffer *buffer, InsertData *data);
...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(
    PlumaBookmarksPlugin, pluma_bookmarks_plugin, PEAS_TYPE_EXTENSION_BASE, 0,
...
static void pluma_bookmarks_plugin_init(PlumaBookmarksPlugin *plugin) {
...
static void pluma_bookmarks_plugin_dispose(GObject *object) {
...
static void pluma_bookmarks_plugin_set_property(GObject *object, guint prop_id,
                                                const GValue *value,
                                                GParamSpec *pspec) {
  PlumaBookmarksPlugin *plugin = PLUMA_BOOKMARKS_PLUGIN(object);
...
static void pluma_bookmarks_plugin_get_property(GObject *object, guint prop_id,
                                                GValue *value,
                                                GParamSpec *pspec) {
  PlumaBookmarksPlugin *plugin = PLUMA_BOOKMARKS_PLUGIN(object);
...
static GtkActionEntry const action_entries[] = {
    ...
    {"NextBookmark", NULL, N_("Go to Next Bookmark"), "<Control>B",
     N_("Go to the next bookmark"), G_CALLBACK(on_next_bookmark_activate)},
    {"PreviousBookmark", NULL, N_("Go to Previous Bookmark"),
     "<Control><Shift>B", N_("Go to the previous bookmark"),
...
static void install_menu(PlumaBookmarksPlugin *plugin) {
  ...
  priv->ui_id =
      gtk_ui_manager_add_ui_from_string(manager, uidefinition, -1, &error);
...
static void uninstall_menu(PlumaBookmarksPlugin *plugin) {
  PlumaBookmarksPluginPrivate *priv;
  GtkUIManager *manager;
...
static void remove_all_bookmarks(GtkSourceBuffer *buffer) {
  GtkTextIter start;
  GtkTextIter end;
  ...
  gtk_source_buffer_remove_source_marks(buffer, &start, &end,
                                        BOOKMARK_CATEGORY);
}

static void disable_bookmarks(PlumaView *view) {
...
static GdkPixbuf *get_bookmark_pixbuf(PlumaBookmarksPlugin *plugin) {
  GdkPixbuf *pixbuf;
  gint width;
  GError *error = NULL;
  ...
  pixbuf = gtk_icon_theme_load_icon(gtk_icon_theme_get_default(),
                                    "user-bookmarks-symbolic", (width * 2) / 3,
                                    0, &error);
...
static void update_background_color(GtkSourceMarkAttributes *attrs,
...
static void enable_bookmarks(PlumaView *view, PlumaBookmarksPlugin *plugin) {
  GdkPixbuf *pixbuf;
  ...
  if (pixbuf) {
    ...
    gtk_source_view_set_mark_attributes(
        GTK_SOURCE_VIEW(view), BOOKMARK_CATEGORY, attrs, BOOKMARK_PRIORITY);

    gtk_source_view_set_show_line_marks(GTK_SOURCE_VIEW(view), TRUE);

    g_signal_connect(buffer, "notify::style-scheme",
                     G_CALLBACK(on_style_scheme_notify), view);

    g_signal_connect(buffer, "delete-range", G_CALLBACK(on_delete_range), NULL);

    data = g_slice_new0(InsertData);

    g_object_set_data_full(G_OBJECT(buffer), INSERT_DATA_KEY, data,
                           (GDestroyNotify)free_insert_data);

    g_signal_connect(buffer, "insert-text", G_CALLBACK(on_insert_text_before),
                     data);

    g_signal_connect(buffer, "begin-user-action",
                     G_CALLBACK(on_begin_user_action), data);

    g_signal_connect(buffer, "end-user-action", G_CALLBACK(on_end_user_action),
                     data);
...
static void load_bookmarks(PlumaView *view, gchar **bookmarks) {
  GtkSourceBuffer *buf;
  GtkTextIter iter;
  gint tot_lines;
  gint i;
  ...
  for (i = 0; bookmarks != NULL && bookmarks[i] != NULL; i++) {
    ...
    if (line >= 0 && line < tot_lines) {
      ...
      gtk_text_buffer_get_iter_at_line(GTK_TEXT_BUFFER(buf), &iter, line);

      marks = gtk_source_buffer_get_source_marks_at_iter(buf, &iter,
                                                         BOOKMARK_CATEGORY);
      if (marks == NULL) /* Add new bookmark */
        gtk_source_buffer_create_source_mark(buf, NULL, BOOKMARK_CATEGORY,
                                             &iter);
      else
        g_slist_free(marks);
    }
...
static void load_bookmark_metadata(PlumaView *view) {
  PlumaDocument *doc;
...
typedef gboolean (*IterSearchFunc)(GtkSourceBuffer *buffer, GtkTextIter *iter,
                                   const gchar *category);
typedef void (*CycleFunc)(GtkTextBuffer *buffer, GtkTextIter *iter);

static void goto_bookmark(PlumaWindow *window, GtkSourceView *view,
                          GtkTextIter *iter, IterSearchFunc func,
  ...
  if (!func(GTK_SOURCE_BUFFER(buffer), &at, BOOKMARK_CATEGORY)) {
    ...
    marks = gtk_source_buffer_get_source_marks_at_iter(
        GTK_SOURCE_BUFFER(buffer), &at, BOOKMARK_CATEGORY);
...
static void message_get_view_iter(PlumaWindow *window, PlumaMessage *message,
                                  GtkSourceView **view, GtkTextIter *iter) {
...
static void message_toggle_cb(PlumaMessageBus *bus, PlumaMessage *message,
                              PlumaWindow *window) {
  ...
  toggle_bookmark(
      GTK_SOURCE_BUFFER(gtk_text_view_get_buffer(GTK_TEXT_VIEW(view))), &iter);
}

static void message_add_cb(PlumaMessageBus *bus, PlumaMessage *message,
                           PlumaWindow *window) {
  GtkSourceView *view = NULL;
  GtkTextIter iter;
...
static void message_remove_cb(PlumaMessageBus *bus, PlumaMessage *message,
                              PlumaWindow *window) {
  GtkSourceView *view = NULL;
  GtkTextIter iter;
...
static void message_goto_next_cb(PlumaMessageBus *bus, PlumaMessage *message,
                                 PlumaWindow *window) {
  GtkSourceView *view = NULL;
  GtkTextIter iter;
  ...
  goto_bookmark(window, view, &iter,
                gtk_source_buffer_forward_iter_to_source_mark,
                gtk_text_buffer_get_start_iter);
}

static void message_goto_previous_cb(PlumaMessageBus *bus,
...
static void install_messages(PlumaWindow *window) {
  PlumaMessageBus *bus = pluma_window_get_message_bus(window);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "toggle", 2, "view",
                             GTK_SOURCE_TYPE_VIEW, "iter", GTK_TYPE_TEXT_ITER,
                             NULL);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "add", 2, "view",
                             GTK_SOURCE_TYPE_VIEW, "iter", GTK_TYPE_TEXT_ITER,
                             NULL);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "remove", 2, "view",
                             GTK_SOURCE_TYPE_VIEW, "iter", GTK_TYPE_TEXT_ITER,
                             NULL);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "goto_next", 2, "view",
                             GTK_SOURCE_TYPE_VIEW, "iter", GTK_TYPE_TEXT_ITER,
                             NULL);

  pluma_message_bus_register(bus, MESSAGE_OBJECT_PATH, "goto_previous", 2,
                             "view", GTK_SOURCE_TYPE_VIEW, "iter",
                             GTK_TYPE_TEXT_ITER, NULL);
...
static void uninstall_messages(PlumaWindow *window) {
  PlumaMessageBus *bus = pluma_window_get_message_bus(window);
  pluma_message_bus_unregister_all(bus, MESSAGE_OBJECT_PATH);
}

static void pluma_bookmarks_plugin_activate(
  ...
  g_signal_connect(priv->window, "tab-added", G_CALLBACK(on_tab_added),
                   activatable);

  g_signal_connect(priv->window, "tab-removed", G_CALLBACK(on_tab_removed),
                   activatable);
...
static void pluma_bookmarks_plugin_update_state(
    PlumaWindowActivatable *activatable) {
  PlumaBookmarksPluginPrivate *priv;

  priv = PLUMA_BOOKMARKS_PLUGIN(activatable)->priv;
  gtk_action_group_set_sensitive(
      priv->action_group, pluma_window_get_active_view(priv->window) != NULL);
}

static void save_bookmark_metadata(PlumaView *view) {
  GtkTextIter iter;
  GtkTextBuffer *buf;
  GString *string;
  gchar *val = NULL;
  ...
  while (gtk_source_buffer_forward_iter_to_source_mark(
      GTK_SOURCE_BUFFER(buf), &iter, BOOKMARK_CATEGORY)) {
    gint line;
  ...
  pluma_document_set_metadata(PLUMA_DOCUMENT(buf), METADATA_ATTR, val, NULL);

  g_free(val);
}

static void pluma_bookmarks_plugin_deactivate(
    PlumaWindowActivatable *activatable) {
  PlumaBookmarksPluginPrivate *priv;
  GList *views;
  GList *item;
  ...
  g_signal_handlers_disconnect_by_func(priv->window, on_tab_removed,
                                       activatable);
}

static void pluma_bookmarks_plugin_class_init(
    PlumaBookmarksPluginClass *klass) {
...
static void pluma_bookmarks_plugin_class_finalize(
    PlumaBookmarksPluginClass *klass) {}
...
static void on_delete_range(GtkTextBuffer *buffer, GtkTextIter *start,
  ...
  if (gtk_text_iter_get_line(start) == gtk_text_iter_get_line(end)) return;
  ...
  keep_bookmark =
      ((gtk_source_buffer_get_source_marks_at_iter(
            GTK_SOURCE_BUFFER(buffer), &start_iter, BOOKMARK_CATEGORY) !=
        NULL) ||
       (gtk_source_buffer_get_source_marks_at_iter(
            GTK_SOURCE_BUFFER(buffer), &end_iter, BOOKMARK_CATEGORY) != NULL));

  /* Remove all bookmarks in the range. */
  gtk_source_buffer_remove_source_marks(GTK_SOURCE_BUFFER(buffer), &start_iter,
                                        &end_iter, BOOKMARK_CATEGORY);

  if (keep_bookmark)
    gtk_source_buffer_create_source_mark(GTK_SOURCE_BUFFER(buffer), NULL,
                                         BOOKMARK_CATEGORY, &start_iter);
}
...
static void on_end_user_action(GtkTextBuffer *buffer, InsertData *data) {
  ...
  for (item = data->trackers; item; item = g_slist_next(item)) {
    ...
    gtk_text_buffer_get_iter_at_mark(buffer, &curloc,
                                     GTK_TEXT_MARK(tracker->bookmark));

    gtk_text_buffer_get_iter_at_mark(buffer, &newloc, tracker->mark);

    if (gtk_text_iter_get_line(&curloc) != gtk_text_iter_get_line(&newloc)) {
      gtk_text_iter_set_line_offset(&newloc, 0);
      gtk_text_buffer_move_mark(buffer, GTK_TEXT_MARK(tracker->bookmark),
                                &newloc);
    }
...
static void add_tracker(GtkTextBuffer *buffer, GtkTextIter *iter,
                        GtkSourceMark *bookmark, InsertData *data) {
  GSList *item;
  ...
  for (item = data->trackers; item; item = g_slist_next(item)) {
    ...
    if (tracker->bookmark == bookmark) return;
  }

  tracker = g_slice_new(InsertTracker);
  tracker->bookmark = bookmark;
  tracker->mark = gtk_text_buffer_create_mark(buffer, NULL, iter, FALSE);
...
static void on_insert_text_before(GtkTextBuffer *buffer, GtkTextIter *location,
  ...
  if (gtk_text_iter_starts_line(location)) {
    GSList *marks;
    marks = gtk_source_buffer_get_source_marks_at_iter(
        GTK_SOURCE_BUFFER(buffer), location, BOOKMARK_CATEGORY);
...
static GtkSourceMark *get_bookmark_and_iter(GtkSourceBuffer *buffer,
                                            GtkTextIter *iter,
                                            GtkTextIter *start) {
  GSList *marks;
  ...
  marks = gtk_source_buffer_get_source_marks_at_iter(buffer, start,
                                                     BOOKMARK_CATEGORY);
  if (marks != NULL) ret = GTK_SOURCE_MARK(marks->data);
...
static void on_toggle_bookmark_activate(GtkAction *action,
                                        PlumaBookmarksPlugin *plugin) {
  toggle_bookmark(
      GTK_SOURCE_BUFFER(pluma_window_get_active_document(plugin->priv->window)),
      NULL);
}

static void on_next_bookmark_activate(GtkAction *action,
                                      PlumaBookmarksPlugin *plugin) {
  goto_bookmark(plugin->priv->window, NULL, NULL,
                gtk_source_buffer_forward_iter_to_source_mark,
                gtk_text_buffer_get_start_iter);
}
...
static void on_document_saved(PlumaDocument *doc, const GError *error,
                              PlumaView *view) {
  if (error == NULL) save_bookmark_metadata(view);
}
...
static void on_tab_added(PlumaWindow *window, PlumaTab *tab,
  ...
  g_signal_connect(doc, "loaded", G_CALLBACK(on_document_loaded), view);
  g_signal_connect(doc, "saved", G_CALLBACK(on_document_saved), view);
...
G_MODULE_EXPORT void peas_register_types(PeasObjectModule *module) {
  ...
  peas_object_module_register_extension_type(
      module, PLUMA_TYPE_WINDOW_ACTIVATABLE, PLUMA_TYPE_BOOKMARKS_PLUGIN);
}
