...
static void pluma_view_activatable_iface_init(
...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(
    PlumaWordCompletionPlugin, pluma_word_completion_plugin,
...
enum { PROP_0, PROP_WINDOW, PROP_VIEW };

static void pluma_word_completion_plugin_init(
    PlumaWordCompletionPlugin *plugin) {
...
static void pluma_word_completion_plugin_dispose(GObject *object) {
...
static void pluma_word_completion_plugin_set_property(GObject *object,
                                                      guint prop_id,
                                                      const GValue *value,
                                                      GParamSpec *pspec) {
  PlumaWordCompletionPlugin *plugin = PLUMA_WORD_COMPLETION_PLUGIN(object);
...
static void pluma_word_completion_plugin_get_property(GObject *object,
                                                      guint prop_id,
                                                      GValue *value,
                                                      GParamSpec *pspec) {
  PlumaWordCompletionPlugin *plugin = PLUMA_WORD_COMPLETION_PLUGIN(object);
...
static void update_activation(GtkSourceCompletionWords *provider,
                              GSettings *settings) {
...
static void on_interactive_completion_changed_cb(
    GSettings *settings, gchar *key, GtkSourceCompletionWords *provider) {
  update_activation(provider, settings);
}

static GtkSourceCompletionWords *create_provider(void) {
  ...
  g_settings_bind(settings, SETTINGS_KEY_MINIMUM_WORD_SIZE, provider,
                  "minimum-word-size", G_SETTINGS_BIND_GET);

  update_activation(provider, settings);

  g_signal_connect_object(
      settings, "changed::" SETTINGS_KEY_INTERACTIVE_COMPLETION,
      G_CALLBACK(on_interactive_completion_changed_cb), provider, 0);
...
static void pluma_word_completion_window_activate(
  ...
  g_object_set_data_full(G_OBJECT(priv->window), WINDOW_PROVIDER, provider,
                         (GDestroyNotify)g_object_unref);
}

static void pluma_word_completion_window_deactivate(
    PlumaWindowActivatable *activatable) {
  PlumaWordCompletionPluginPrivate *priv;
...
static void pluma_word_completion_view_activate(
    PlumaViewActivatable *activatable) {
  PlumaWordCompletionPluginPrivate *priv;
...
static void pluma_word_completion_view_deactivate(
    PlumaViewActivatable *activatable) {
  PlumaWordCompletionPluginPrivate *priv;
  GtkSourceCompletion *completion;
  GtkTextBuffer *buf;
  ...
  gtk_source_completion_remove_provider(completion, priv->provider, NULL);

  gtk_source_completion_words_unregister(
      GTK_SOURCE_COMPLETION_WORDS(priv->provider), buf);
}

static GtkWidget *pluma_word_completion_create_configure_widget(
  ...
  data_dir =
      peas_extension_base_get_data_dir(PEAS_EXTENSION_BASE(configurable));
  ui_file =
      g_build_filename(data_dir, "pluma-word-completion-configure.ui", NULL);

  builder = gtk_builder_new_from_resource(
      "/org/mate/pluma/plugins/wordcompletion/ui/"
  ...
  interactive_completion = GTK_WIDGET(
      gtk_builder_get_object(builder, "check_button_interactive_completion"));
  min_word_size =
      GTK_WIDGET(gtk_builder_get_object(builder, "spin_button_min_word_size"));
  ...
  g_settings_bind(settings, SETTINGS_KEY_MINIMUM_WORD_SIZE, min_word_size,
                  "value",
                  G_SETTINGS_BIND_DEFAULT | G_SETTINGS_BIND_GET_NO_CHANGES);
...
static void pluma_word_completion_plugin_class_init(
    PlumaWordCompletionPluginClass *klass) {
...
static void pluma_word_completion_plugin_class_finalize(
    PlumaWordCompletionPluginClass *klass) {}
...
static void peas_gtk_configurable_iface_init(
    PeasGtkConfigurableInterface *iface) {
  iface->create_configure_widget =
      pluma_word_completion_create_configure_widget;
}
...
G_MODULE_EXPORT void peas_register_types(PeasObjectModule *module) {
  ...
  peas_object_module_register_extension_type(
      module, PLUMA_TYPE_WINDOW_ACTIVATABLE, PLUMA_TYPE_WORD_COMPLETION_PLUGIN);

  peas_object_module_register_extension_type(
      module, PLUMA_TYPE_VIEW_ACTIVATABLE, PLUMA_TYPE_WORD_COMPLETION_PLUGIN);
