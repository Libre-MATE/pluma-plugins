...
G_DEFINE_DYNAMIC_TYPE_EXTENDED(
    PlumaQuickHighlightPlugin, pluma_quick_highlight_plugin,
...
static void pluma_quick_highlight_plugin_notify_buffer_cb(GObject *object,
                                                          GParamSpec *pspec,
                                                          gpointer user_data);
static void pluma_quick_highlight_plugin_mark_set_cb(GtkTextBuffer *textbuffer,
                                                     GtkTextIter *location,
                                                     GtkTextMark *mark,
                                                     gpointer user_data);
static void pluma_quick_highlight_plugin_delete_range_cb(
    GtkTextBuffer *textbuffer, GtkTextIter *start, GtkTextIter *end,
    gpointer user_data);
static void pluma_quick_highlight_plugin_notify_style_scheme_cb(
    GObject *object, GParamSpec *pspec, gpointer user_data);

static void pluma_quick_highlight_plugin_load_style(
...
static gboolean pluma_quick_highlight_plugin_highlight_worker(
    gpointer user_data) {
  ...
  if (plugin->priv->search_context == NULL) {
    ...
    plugin->priv->search_context =
        g_object_new(GTK_SOURCE_TYPE_SEARCH_CONTEXT, "buffer",
                     plugin->priv->buffer, "highlight", FALSE, "match-style",
                     plugin->priv->style, "settings", search_settings, NULL);
...
static void pluma_quick_highlight_plugin_queue_update(
    PlumaQuickHighlightPlugin *plugin) {
  g_return_if_fail(PLUMA_IS_QUICK_HIGHLIGHT_PLUGIN(plugin));
  ...
  plugin->priv->queued_highlight = gdk_threads_add_idle_full(
      G_PRIORITY_LOW, pluma_quick_highlight_plugin_highlight_worker,
      g_object_ref(plugin), g_object_unref);
}

static void pluma_quick_highlight_plugin_notify_weak_buffer_cb(
...
static void pluma_quick_highlight_plugin_unref_weak_buffer(
    PlumaQuickHighlightPlugin *plugin) {
  g_return_if_fail(PLUMA_IS_QUICK_HIGHLIGHT_PLUGIN(plugin));
...
static void pluma_quick_highlight_plugin_set_buffer(
    PlumaQuickHighlightPlugin *plugin, PlumaDocument *buffer) {
  g_return_if_fail(PLUMA_IS_QUICK_HIGHLIGHT_PLUGIN(plugin));
  ...
  if (plugin->priv->buffer != NULL) {
    ...
    plugin->priv->mark_set_handler_id = g_signal_connect(
        ...
        G_CALLBACK(pluma_quick_highlight_plugin_mark_set_cb), plugin);

    plugin->priv->delete_range_handler_id = g_signal_connect(
        plugin->priv->buffer, "delete-range",
        G_CALLBACK(pluma_quick_highlight_plugin_delete_range_cb), plugin);
...
static void pluma_quick_highlight_plugin_dispose(GObject *object) {
...
static void pluma_quick_highlight_plugin_finalize(GObject *object) {
  PlumaQuickHighlightPlugin *plugin = PLUMA_QUICK_HIGHLIGHT_PLUGIN(object);
...
static void pluma_quick_highlight_plugin_get_property(GObject *object,
                                                      guint prop_id,
                                                      GValue *value,
                                                      GParamSpec *pspec) {
  PlumaQuickHighlightPlugin *plugin = PLUMA_QUICK_HIGHLIGHT_PLUGIN(object);
...
static void pluma_quick_highlight_plugin_set_property(GObject *object,
                                                      guint prop_id,
                                                      const GValue *value,
                                                      GParamSpec *pspec) {
  PlumaQuickHighlightPlugin *plugin = PLUMA_QUICK_HIGHLIGHT_PLUGIN(object);
...
static void pluma_quick_highlight_plugin_class_init(
    PlumaQuickHighlightPluginClass *klass) {
...
static void pluma_quick_highlight_plugin_class_finalize(
    PlumaQuickHighlightPluginClass *klass) {}

static void pluma_quick_highlight_plugin_init(
    PlumaQuickHighlightPlugin *plugin) {
  plugin->priv = pluma_quick_highlight_plugin_get_instance_private(plugin);
}

static void pluma_quick_highlight_plugin_activate(
  ...
  plugin->priv->buffer_handler_id = g_signal_connect(
      ...
      G_CALLBACK(pluma_quick_highlight_plugin_notify_buffer_cb), plugin);
...
static void pluma_quick_highlight_plugin_deactivate(
    PlumaViewActivatable *activatable) {
  PlumaQuickHighlightPlugin *plugin;
...
G_MODULE_EXPORT void peas_register_types(PeasObjectModule *module) {
  ...
  peas_object_module_register_extension_type(
      module, PLUMA_TYPE_VIEW_ACTIVATABLE, PLUMA_TYPE_QUICK_HIGHLIGHT_PLUGIN);
}
